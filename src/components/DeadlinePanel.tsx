...
const DeadlinePanel = ({ deadlines }: DeadlinePanelProps) => {
  return (
    <div className="glass rounded-xl sm:rounded-2xl p-4 sm:p-6 animate-fade-in">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-2 sm:gap-3">
        {deadlines.map((deadline) => (
          <div
            key={deadline.id}
            className={`p-3 sm:p-4 rounded-lg sm:rounded-xl bg-secondary/50 border transition-all duration-200 hover:bg-secondary/70 cursor-pointer group relative overflow-hidden ${
              deadline.urgent
                ? "border-orange-500/40 hover:border-orange-500/60 animate-pulse"
          ...
          >
            ...
            <div className="flex items-start justify-between gap-2 mb-1.5 sm:mb-2 relative z-10">
              <span className={`text-[10px] sm:text-xs px-1.5 sm:px-2 py-0.5 rounded-full flex items-center gap-1 ${
                deadline.urgent 
              ...
              {deadline.urgent && (
                <AlertCircle className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-orange-400 shrink-0 animate-pulse" />
              )}
            </div>
